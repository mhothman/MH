...
def _export_budget_to_csv(report_data: dict) -> StreamingResponse:
    ...
    writer.writerow(["Project", "Budget", "Spent", "Remaining", "Status", "Currency", "Spent %"])
    ...
    return StreamingResponse(
        io.BytesIO(output.getvalue().encode('utf-8')),
        media_type="text/csv",
        headers={"Content-Disposition": f"attachment; filename=budget_summary_{datetime.now().strftime('%Y%m%d')}.csv"}
    )
...
def _export_projects_to_csv(report_data: dict) -> StreamingResponse:
    ...
    writer.writerow(["Project", "Status", "Tasks", "Completed", "In Progress", "To Do", "Completion %"])
